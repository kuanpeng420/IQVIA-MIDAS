...
#### Author: Kuan Peng 
#### Created time: 2021/9/13
#### Last revision: 2023/1/2
...
MIDAS <- data.table(read_excel("MIDAS.new.xlsx",sheet = 3,guess_max = 10000,progress = TRUE))

ES <- MIDAS[`Molecule List`%in%"INFLIXIMAB"]#Filter Infliximab
ES <- ES[!str_detect(Panel,regex("Russian",ignore_case = TRUE))]#Remove russian for not being asia-pacific country
...
ES_s_o <- ES_s_o[,nation:=countries]#Assign the correct country name
...
for (i in 1:length(test)) {
        tt <- unlist(test[[i]])[1]
        countries <- c(countries,tt)#Assign the correct country name
}
...
ES_p[,`conversion factor`:=1]#standardized to 2020 USD https://www.usinflationcalculator.com/
...
es_remove <- ES_p[price<=10&str_detect(nation,"AUSTRALIA")]#Remove products with unit price lower than 10, which is commerical promotion
...
a1/a2*100#less than 0.01% overall consumption data was removed
...
#Standardized consumption by population level
...
# Interrupted time series on price----
...
#Supplementary analysis on Medicaid data
...
r <- list("Hong Kong"=r1,"India"=r2,"Japan"=r3,"Korea"=r4,
        "Us"=r8,"Uk"=r9,"Canada"=r10,"Australia"=r11)
...
# Interruptted time series on sales ----
ES_ITS1 <- ES_eli[nation=="HONG"]
ES_ITS1 <-ES_ITS1[,time:=1:nrow(ES_ITS1)][,Biosimilarintro:=ifelse(quarter_sale_Y==0,0,1)]
b=44-sum(ES_ITS1$Biosimilarintro==1)
ES_ITS1 <- ES_ITS1[,time_after_inter:=ifelse(time<=b,0,time-b)]
